...
import com.ch.cloud.redis.entity.Cluster;
import com.ch.cloud.redis.entity.NodeRole;
...
import com.ch.cloud.redis.utils.RedisUtil;
import com.ch.cloud.redis.utils.SignUtil;
...
public class RedisClient implements IRedisClient {
    ...
    public RedisClient(RedisURI redisURI) {
        ...
        if(StringUtils.isNotBlank(redisURI.getSentinelMasterId())) {
            try {
                JedisSentinelPool.createJedisPool(redisURI);
                jedis = JedisSentinelPool.getJedis();
        ...
        for (HostAndPort hostAndPort : hostAndPortSet) {
            try {
                jedis = new Jedis(hostAndPort.getHost(), hostAndPort.getPort(), RedisURI.TIMEOUT, RedisURI.TIMEOUT);
    ...
    public List<RedisNode> nodes() throws Exception {
        ...
        String role = infoMap.get(RedisUtil.ROLE);
        String masterHost;
        Integer masterPort;
        // 使用 master node 进行连接
        if (Objects.equals(role, NodeRole.SLAVE.getValue())) {
            close();
            masterHost = infoMap.get(RedisUtil.MASTER_HOST);
            masterPort = Integer.parseInt(infoMap.get(RedisUtil.MASTER_PORT));
        ...
        for (Map.Entry<String, String> node : infoMap.entrySet()) {
            ...
            for (String keyAndValue : keyAndValues) {
                ...
                if (Objects.equals(key, RedisUtil.IP)) {
                    slaveIp = val;
                } else if (Objects.equals(key, RedisUtil.PORT)) {
                    slavePort = val;
                }
            }
    ...
    public Object hash(DataCommandsParam dataCommandsParam) {
        ...
        if (cmd.startsWith(HGETALL)) {
            ...
            String[] items = RedisUtil.removeCommandAndKey(list);
    ...
    public Object set(DataCommandsParam dataCommandsParam) {
        ...
        if (cmd.startsWith(SCARD)) {
            ...
            result = jedis.sadd(key, RedisUtil.removeCommandAndKey(list));
