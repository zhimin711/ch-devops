...
package com.ch.cloud.rocketmq.task;
...
@Component
//@ConditionalOnProperty(prefix = "rocketmq.namesrv.consoleCollect", value = {"true"})
@Slf4j
public class DashboardCollectTask {
    ...
    public void collectTopic() {
        ...
        if (CommonUtils.isEmpty(rmqConfigure.getClients())) {
            log.info("no client");
            return;
        }
        rmqConfigure.getClients().forEach(client -> {
            try {
                ...
                List<RMQTopicCollect> list = Lists.newArrayList();
                for (String topic : topicSet) {
                    try {
                        if (topic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX) || topic.startsWith(
                        ...
                        RMQTopicCollect dto = new RMQTopicCollect(null, client.getAddr(), topic, date, date,
                                new BigDecimal(inTPS), inMsgCntToday, new BigDecimal(outTPS), outMsgCntToday);
                        list.add(dto);
                    } catch (Exception err) {
                        log.error("collect topic error", err);
                    }
                }
                if (!list.isEmpty()) {
                    dashboardCollectService.saveBatchTopicData(list);
                }
            } catch (Exception e) {
                log.error("collect topic error", e);
            } finally {
    ...
    public void collectBroker() {
        ...
        rmqConfigure.getClients().forEach(client -> {
            try {
                ...
                List<RMQBrokerCollect> list = Lists.newArrayList();
                ...
                for (Map.Entry<String, String> entry : entries) {
                    KVTable kvTable = fetchBrokerRuntimeStats(mqAdminExt, entry.getKey(), 3);
                    if (kvTable == null) {
                        continue;
                    }
                    ...
                    RMQBrokerCollect dto = new RMQBrokerCollect(null, client.getAddr(), entry.getValue(), date, date,
                            averageTps);
                    list.add(dto);
                }
                if (!list.isEmpty()) {
                    dashboardCollectService.saveBatchBrokerData(list);
                }
            } catch (Exception e) {
                log.error("collect broker error", e);
            } finally {
                RMQAdminUtil.destroyMQAdminExt();
            }
        });
    ...
    private KVTable fetchBrokerRuntimeStats(MQAdminExt mqAdminExt, String brokerAddr, Integer retryTime) {
        if (retryTime == 0) {
            return null;
        }
        try {
            ...
            fetchBrokerRuntimeStats(mqAdminExt, brokerAddr, retryTime - 1);
            throw Throwables.propagate(e);
        }
    }
